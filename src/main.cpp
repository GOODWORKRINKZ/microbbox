...
void setup() {
    ...
    if (FirmwareUpdate::isOTAPending()) {
        ...
        // initSafeModeForOTA() всегда вызывает ESP.restart(), поэтому этот код не достигается
        // Однако if (!robot->initSafeModeForOTA()) выше перехватывает ошибки
        // и также вызывает ESP.restart(), так что в любом случае мы перезагружаемся
        while(1) {
            delay(1000);  // Ждем перезагрузки (на всякий случай)
        }
    }
