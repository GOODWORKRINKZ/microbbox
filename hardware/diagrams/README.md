...
> **✅ Статус**: Схемы экспортированы из Fritzing!

Эта папка содержит графические схемы, диаграммы подключения и визуальную документацию проекта МикРоББокс.

## 📋 Доступные материалы

### Схемы из Fritzing (SVG)

**✅ Экспортированные схемы:**

1. **microbbox_breadboard.svg** (~150KB)
   - Визуальная схема монтажа на макетной плате
   - Показывает все компоненты и соединения наглядно
   - Идеально для сборки и понимания подключений
   - Все компоненты с реалистичным внешним видом

2. **microbbox_schematic.svg** (~64KB)
   - Классическая принципиальная электрическая схема
   - Стандартные символы электронных компонентов
   - Показывает логические соединения между элементами
   - Подходит для технического анализа

3. **microbbox_pcb.svg** (~63KB)
   - Схема разводки печатной платы
   - Показывает физическое размещение компонентов
   - Может использоваться как основа для PCB

**Компоненты в схемах:**
- ESP32-CAM (AI Thinker)
- MX1508 DC Motor Driver  
- Mini-360 Buck Converter (5V output)
- 2x N-20 DC Motors (300rpm)
- 9V Battery (Li-Ion 1100mAh)
- 3x WS2812B LEDs
- Соединительные провода

### Исходные файлы Fritzing
...
4. **microbbox.fzz** (~116KB)
   - Исходный файл проекта Fritzing
   - Открывается в [Fritzing](https://fritzing.org/)
   - Содержит все 3 вида (Breadboard, Schematic, PCB)
   - Можно редактировать и экспортировать в другие форматы

5. **microbbox.fzb** (~471 bytes)
   - Бинарный файл метаданных Fritzing
...
**SVG схемы** (`microbbox_breadboard.svg`, `microbbox_schematic.svg`, `microbbox_pcb.svg`):
- Открываются в любом современном браузере
- Можно просмотреть прямо на GitHub
- Масштабируются без потери качества
- **Рекомендуется**: `microbbox_breadboard.svg` для сборки

**Какую схему использовать:**
- **Breadboard** - Для визуальной сборки, показывает как выглядят компоненты
- **Schematic** - Для понимания логики схемы и анализа
- **PCB** - Для создания печатной платы (опционально)

**Fritzing файлы**:
- Требуется установка [Fritzing](https://fritzing.org/)
- Откройте `microbbox.fzz` для просмотра и редактирования
- Можно экспортировать в PNG, PDF, SVG, Gerber

### Редактирование схем

#### Экспорт из Fritzing (.fzz)
```bash
# Установка Fritzing
...
# Экспорт в SVG (уже сделано):
xvfb-run -a fritzing -svg /path/to/folder
```

Открыв `fritzing/microbbox.fzz` в Fritzing, можно:
- Редактировать схему
- Добавлять/удалять компоненты
- Экспортировать в различные форматы (PNG, PDF, SVG, Gerber)
- Создать файлы для производства PCB
