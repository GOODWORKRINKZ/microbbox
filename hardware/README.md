...
> **⚠️ В разработке**: STL файлы будут добавлены в следующих версиях.

Планируемые модели:
- `chassis.stl` - Основное шасси робота
- `wheel.stl` - Колеса для моторов (x2)
- `front_bumper.stl` - Передний бампер
- `battery_holder.stl` - Держатель аккумулятора
- `pcb_mount.stl` - Крепление для ESP32-CAM
- `led_diffuser.stl` - Рассеиватели для NeoPixel LED

### 📊 Диаграммы

Папка **`diagrams/`** содержит схемы и диаграммы:

> **⚠️ В разработке**: Графические схемы будут добавлены в следующих версиях.

Планируемые диаграммы:
- `connection_schematic.png` - Схема подключения (Fritzing/KiCad)
- `pcb_layout.png` - Разводка печатной платы (опционально)
- `assembly_guide.pdf` - Пошаговая инструкция по сборке
...
> **Временное решение**: Пока STL файлы в разработке, можно:
> - Использовать готовые шасси для роботов с колесной базой ~80-100мм
> - Изготовить простое шасси из акрила/фанеры
> - Использовать монтажную плату для прототипа

Параметры печати (когда STL будут готовы):
- **Материал**: PLA или PETG
- **Заполнение**: 20-30%
- **Толщина слоя**: 0.2мм
- **Поддержки**: Да (для сложных деталей)
...
> Детальная инструкция будет добавлена после создания STL файлов

Общие этапы:
1. Печать всех деталей из 3D-моделей
2. Установка моторов в шасси
3. Крепление ESP32-CAM и MX1508
4. Установка аккумулятора
5. Монтаж колес
6. Установка светодиодов и бузера
...
| STL файлы | 🚧 В разработке | Планируется v1.1 |
| Графические схемы | 🚧 В разработке | Планируется v1.1 |
| Инструкция по сборке | 🚧 В разработке | Планируется v1.1 |
