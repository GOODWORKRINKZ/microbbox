...
КАЛИБРОВКА:
Перед началом тестирования можно загрузить калибровочное изображение
(белое поле без линий). Калибровочные значения вычитаются из входных кадров,
что убирает неравномерность освещения от фонаря и усиливает сигнал линии.

Структура данных:
data/
  calibration.jpg    - калибровочное изображение (белое поле без линий)
...
# Глобальные переменные для калибровочных данных
calibration_lines = None  # Массив 4×160 для хранения калибровочных линий
...
def load_calibration(calibration_path):
    """
    Загружает калибровочное изображение (белое поле без линий).
    
    Калибровочное изображение должно быть сделано на белом поле БЕЗ линий.
    Сохраняются значения яркости для 4 сканирующих линий (по 160 пикселей каждая).
    При детектировании эти значения вычитаются из входного кадра, что убирает
    неравномерность освещения от фонаря и усиливает сигнал темной линии.
    
    Args:
        calibration_path: путь к калибровочному изображению
    
    Returns:
        bool: True если калибровка загружена успешно
    """
    global calibration_lines, has_calibration
    
    if not os.path.exists(calibration_path):
        print(f"⚠️ Калибровочное изображение не найдено: {calibration_path}")
        has_calibration = False
        return False
    
    print(f"📸 Загрузка калибровки из {calibration_path}")
    
    # Загрузка изображения
    img = Image.open(calibration_path)
    ...
    # Создаём массив для хранения калибровочных данных (4 линии × 160 пикселей)
    calibration_lines = np.zeros((4, LINE_CAMERA_WIDTH), dtype=np.uint8)
    
    # Копируем значения пикселей для каждой линии
    for scan_idx in range(4):
        y = scan_y[scan_idx]
        calibration_lines[scan_idx, :] = img_array[y, :]
    
    has_calibration = True
    
    print("✓ Калибровка загружена:")
    for scan_idx in range(4):
        avg = np.mean(calibration_lines[scan_idx, :])
        print(f"  Линия {scan_idx} (y={scan_y[scan_idx]}): среднее значение = {avg:.1f}")
...
def detect_line_position(image_path):
    ...
    for scan_idx, scan_y in enumerate(horizontal_scan_heights):
        sum_position = 0.0
        count = 0
...
def main():
    ...
    calibration_path = os.path.join('data', 'calibration.jpg')
    if os.path.exists(calibration_path):
        print("\n📸 Обнаружено калибровочное изображение")
        if load_calibration(calibration_path):
            print("✓ Калибровка активирована")
            print("  При детектировании калибровочные значения будут вычитаться из кадров")
            print("  Это убирает неравномерность освещения и усиливает сигнал линии")
        else:
            print("⚠️ Не удалось загрузить калибровку, продолжаем без неё")
    else:
        print(f"\n📝 Калибровочное изображение не найдено: {calibration_path}")
        print("  Для создания калибровки:")
        print("  1. Поставьте робота на БЕЛОЕ ПОЛЕ БЕЗ ЛИНИЙ")
        print("  2. Сделайте снимок")
        print(f"  3. Сохраните как {calibration_path}")
        print("  Калибровка улучшит детектирование, убрав неравномерность освещения от фонаря")
    ...
    if has_calibration:
        print("✓ Калибровка была применена ко всем изображениям")
    else:
        print("  Калибровка не использовалась")
    
    total_tests = sum(len(results) for results in all_results.values())
    print(f"\nВсего протестировано изображений: {total_tests}")
