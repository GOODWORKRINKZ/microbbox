...
# Физические параметры поля зрения (измерены)
FIELD_WIDTH_MM = 65.0   # Ширина видимого поля в мм
FIELD_HEIGHT_MM = 48.75 # Высота видимого поля в мм (рассчитано: 65 * 120/160)
LINE_WIDTH_MM = 19.0    # Ширина линии в мм
...
PIXELS_PER_MM_X = CAMERA_WIDTH / FIELD_WIDTH_MM   # ≈ 2.46 пикселей/мм
PIXELS_PER_MM_Y = CAMERA_HEIGHT / FIELD_HEIGHT_MM # ≈ 2.46 пикселей/мм
...
# Ширина линии (в пикселях) - рассчитывается из физических размеров
LINE_WIDTH_PX = int(LINE_WIDTH_MM * PIXELS_PER_MM_X)  # ≈ 46.8 пикселей

# Диапазон допустимой ширины линии (±50% от номинала)
MIN_LINE_WIDTH = max(3, int(LINE_WIDTH_PX * 0.25))  # Минимум 25% от номинала
MAX_LINE_WIDTH = int(LINE_WIDTH_PX * 1.5)            # Максимум 150% от номинала
...
def main():
    ...
    print(f"   Поле зрения: {FIELD_WIDTH_MM} × {FIELD_HEIGHT_MM} мм")
    print(f"   Ширина линии: {LINE_WIDTH_MM} мм = {LINE_WIDTH_PX} пикселей")
    print(f"   Пикселей/мм: {PIXELS_PER_MM_X:.2f} (X), {PIXELS_PER_MM_Y:.2f} (Y)")
    print(f"\n🔍 ДИАПАЗОН ДЕТЕКЦИИ:")
    print(f"   Минимальная ширина: {MIN_LINE_WIDTH} пикселей ({MIN_LINE_WIDTH/PIXELS_PER_MM_X:.1f} мм)")
    print(f"   Номинальная ширина: {LINE_WIDTH_PX} пикселей ({LINE_WIDTH_MM} мм)")
    print(f"   Максимальная ширина: {MAX_LINE_WIDTH} пикселей ({MAX_LINE_WIDTH/PIXELS_PER_MM_X:.1f} мм)")
