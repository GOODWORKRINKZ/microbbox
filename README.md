...
### Режим WiFi Client (по умолчанию)
Робот автоматически подключается к сети **ROS2** (пароль: `1234567890`)
1. Убедитесь что сеть ROS2 доступна
2. Робот автоматически подключится к сети
3. IP адрес отобразится в Serial Monitor (115200 baud)
4. Откройте веб-интерфейс по полученному IP адресу

**Настройка сети:**
Измените в `include/config.h`:
```cpp
